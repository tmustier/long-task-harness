#!/usr/bin/env python3
"""
Install Droid hooks for long-task-harness.

Installs project-local hooks in .factory/settings.json that:
...
Note: These hooks are for Factory Droid only. Claude Code uses .claude/settings.json.
"""
...
def get_hooks_config():
    ...
    Uses plain text output (not JSON) which Droid receives as context.
    """
    script_dir = get_script_dir()
    precommit_script = script_dir / "droid_precommit_check.py"
...
def install_hooks(project_dir: Path) -> bool:
    """Install hooks to project's .factory/settings.json."""
    ...
    print("  ✅ Installed session continuity hooks to .factory/settings.json")
    return True


def uninstall_hooks(project_dir: Path) -> bool:
    """Remove long-task-harness hooks from project's .factory/settings.json."""
    settings_path = project_dir / ".factory" / "settings.json"

    if not settings_path.exists():
        print("  ⚠️  No .factory/settings.json found")
        return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Install or uninstall long-task-harness session continuity hooks for Droid"
    )
    parser.add_argument(
        "--uninstall",
        action="store_true",
